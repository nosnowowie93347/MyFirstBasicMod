...
namespace MyFirstBasicMod.NPCs.PuritySpirit
{
    [AutoloadBossHead]
    public class PuritySpirit : ModNPC
    {
        ...
        public override void SetDefaults()
        {
            ...
            music = mod.GetSoundSlot(SoundType.Music, "Sounds/Music/PuritySpirit");
        ...
        public override void AI()
        {
            ...
            {
                Projectile.NewProjectile(npc.Center.X - arenaWidth / 2, npc.Center.Y, NegativeWall.speed, 0f, ModContent.ProjectileType<Projectiles.NegativeWall>(), 0, 0f, Main.myPlayer, npc.whoAmI, arenaHeight);
                stage++;
            }
            ...
            {
                Projectile.NewProjectile(npc.Center.X, npc.Center.Y - arenaHeight / 2, 0f, NegativeWall.speed, ModContent.ProjectileType<Projectiles.NegativeWall>(), 0, 0f, Main.myPlayer, npc.whoAmI, -arenaWidth);
                stage++;
            }
        ...
        public void FindPlayers()
        {
            if (Main.netMode != NetmodeID.MultiplayerClient)
            {
                ...
                for (int k = 0; k < 255; k++)
                {
                    if (Main.player[k].active && Main.player[k].GetModPlayer<PinksPlayer>().heroLives > 0)
                    {
                        targets.Add(k);
                    }
                }
        ...
        public void RunAway()
        {
            ...
            {
                Main.NewText("Hmph. Was that the extent of your power?", 255, 255, 255, true);
            }
        ...
        public void Initialize()
        {
            if (attackProgress == 1)
            {
                ...
                for (int k = 0; k < 255; k++)
                {
                    ...
                    {
                        player.GetModPlayer<PinksPlayer>().heroLives = 3;
            ...
            {
                if (PinksWorld.downedPuritySpirit)
                {
                    Main.NewText("What, you again? Oh well...", 255, 255, 255, true);
                }
                else
                {
                    Main.NewText("You, who have challenged me...", 255, 255, 255, true);
                }
            }
            if (attackProgress == 180)
            {
            ...
            {
                Main.NewText("Show me the power that has saved Terraria!", 255, 255, 255, true);
                attackProgress = 0;
                stage++;
        ...
        private void SetupCrystals(int radius, bool clockwise)
        {
            ...
            for (int k = 0; k < 10; k++)
            {
                ...
                int proj = Projectile.NewProjectile(pos.X, pos.Y, 0f, 0f, ModContent.ProjectileType<Projectiles.PureCrystal>(), damage, 0f, Main.myPlayer, npc.whoAmI, angle);
        ...
        private void UltimateAttack()
        {
            if (attackProgress == 0)
            {
                ...
                if (Main.netMode != NetmodeID.MultiplayerClient)
                {
                    int damage = Main.expertMode ? 90 : 130;
                    Projectile.NewProjectile(npc.Center.X, npc.Center.Y, 0f, 0f, ModContent.ProjectileType<Projectiles.VoidWorld>(), damage, 0f, Main.myPlayer, npc.whoAmI, Main.rand.Next());
                }
            }
            attackProgress++;
        ...
        private void BeamAttack()
        {
            if (attackProgress == 0)
            {
                ...
                int damage = Main.expertMode ? 70 : 81;
                for (int k = 0; k < targets.Count; k++)
                {
                    float x = Main.player[targets[k]].Center.X;
                    Projectile.NewProjectile(x, y, 0f, 0f, ModContent.ProjectileType<Projectiles.PurityBeam>(), damage, 0f, Main.myPlayer, arenaHeight);
                    for (int j = -1; j <= 1; j += 2)
                    {
                        ...
                        Projectile.NewProjectile(spawnX, y, 0f, 0f, ModContent.ProjectileType<Projectiles.PurityBeam>(), damage, 0f, Main.myPlayer, arenaHeight);
                    }
                }
                ...
                {
                    Projectile.NewProjectile(npc.Center.X + Main.rand.Next(-arenaWidth / 2 + 50, arenaWidth / 2 - 50 + 1), y, 0f, 0f, ModContent.ProjectileType<Projectiles.PurityBeam>(), damage, 0f, Main.myPlayer, arenaHeight);
                }
        ...
        private void SnakeAttack()
        {
            if (attackProgress == 0)
            {
                ...
                Projectile.NewProjectile(npc.Center.X, npc.Center.Y, 0f, 0f, ModContent.ProjectileType<Projectiles.PuritySnake>(), damage, 0f, Main.myPlayer, npc.whoAmI, timeMultiplier);
                attackProgress = 240;
            }
            attackProgress--;
            if (attackProgress < 0)
        ...
        private void LaserAttack()
        {
            if (attackProgress == 0)
            {
                ...
                for (int k = 0; k < numAttacks; k++)
                {
                    int proj = Projectile.NewProjectile(npc.Center.X, npc.Center.Y, 0f, 0f, ModContent.ProjectileType<Projectiles.NullLaser>(), damage, 0f, Main.myPlayer, npc.whoAmI, (int)(60f + k * timer));
            ...
            Dust.NewDust(npc.position, npc.width, npc.height, ModContent.DustType<Dusts.Sparkle>(), 0f, 0f, 0, new Color(0, 180, 0), 1.5f);
            attackProgress--;
            if (attackProgress < 0)
            {
                attackProgress = 0;
            ...
            if (attackProgress == 0)
            {
                ...
                for (int j = 0; j < numGroups || j < targets.Count; j++)
                {
                    ...
                    for (int k = 0; k < numSpheres; k++)
                    {
                        ...
                        int proj = Projectile.NewProjectile(pos.X, pos.Y, 0f, 0f, ModContent.ProjectileType<Projectiles.PuritySphere>(), damage, 0f, Main.myPlayer, center.X, center.Y);
        ...
        private void DoShield(int numShields)
        {
            ...
            for (int k = 0; k < 200; k++)
            {
                if (Main.npc[k].active && Main.npc[k].type == ModContent.NPCType<NPCs.PuritySpirit.PurityShield>() && Main.npc[k].ai[0] == npc.whoAmI)
                {
                    count++;
                }
            }
            ...
            if (shieldTimer >= 300 + 300 * timeMult)
            {
                ...
                NPC.NewNPC((int)npc.Center.X, (int)npc.Center.Y + 40, ModContent.NPCType<NPCs.PuritySpirit.PurityShield>(), 0, npc.whoAmI, targetX, targetY);
                shieldTimer = 0;
            }
        }
        ...
        public void FinishFight1()
        {
            ...
            {
                Main.NewText("This is... I thank you for demonstrating your power.", 255, 255, 255, true);
            }
            if (attackProgress >= 240)
            {
                Main.NewText("Please take this as a farewell gift.", 255, 255, 255, true);
                stage++;
                attackProgress = 0;
            }
        }
        ...
        public void FinishFight2()
        {
            ...
            if (attackProgress == 120)
            {
                    ...
                    Main.NewText("I wish you luck in your future farming.", 255, 255, 255, true);
        ...
        public override void NPCLoot()
        {
            ...
            if (!PinksWorld.downedPuritySpirit)
            {
                PinksWorld.downedPuritySpirit = true;
                if (Main.netMode == NetmodeID.Server)
                {
