...
namespace MyFirstBasicMod.Projectiles
{
	...
	public class PurityWisp : HoverShooter
	{
		...
		public override void SetDefaults() {
			...
			inertia = 20f;
			shoot = ModContent.ProjectileType<Projectiles.PurityBolt>();
			shootSpeed = 12f;
		}

		public override void CheckActive() {
			Player player = Main.player[projectile.owner];
			PinksPlayer modPlayer = player.GetModPlayer<PinksPlayer>();
			if (player.dead) {
		...
		public override void CreateDust() {
			if (projectile.ai[0] == 0f) {
				if (Main.rand.NextBool(5)) {
					int dust = Dust.NewDust(projectile.position, projectile.width, projectile.height / 2, ModContent.DustType<Dusts.PuriumFlame>());
				...
				if (Main.rand.NextBool(3)) {
					...
					int dust = Dust.NewDust(projectile.position, projectile.width, projectile.height, ModContent.DustType<Dusts.PuriumFlame>());
